...
/**
 * Created by handsome on 2020/04/01.
 */
public class MyExcelUtil {
    ...
    public static void main(String[] args){
        ...
        //读取excel数据
        ArrayList<Map<String,Object>> result = myExcelUtil.readExcelToObj("C:\\Users\\49522\\Desktop\\大白菜性状调查表（20200205）.xlsx");
        List<CharacterObservation> characterObservationList = new ArrayList<>();
        for(Map<String,Object> map:result){
            //System.out.println(map);
            CharacterObservation characterObservation = null;
            try {
                characterObservation = BeanMapUtils.mapToBean(map, CharacterObservation.class);
            } catch (Exception e) {
                e.printStackTrace();
            }
     ...
     * @param path
     */
    private ArrayList<Map<String,Object>> readExcelToObj(String path) {

        Workbook wb = null;
        ArrayList<Map<String,Object>> result = null;
        try {
            wb = WorkbookFactory.create(new File(path));
            result = readExcel(wb, 0, 20, 0);
     ...
     */
    private ArrayList<Map<String,Object>> readExcel(Workbook wb,int sheetIndex, int startReadLine, int tailLine) {
...
//                    System.out.print(c.getRichStringCellValue()+"++++ ");
                ...
                if(c.getColumnIndex()==0){
                    map.put("observationPeriod",returnStr);
                }else if(c.getColumnIndex()==1){
                    map.put("characteristics",returnStr);
                }else if(c.getColumnIndex()==2){
                    map.put("specificCharacter",returnStr);
                }else if(c.getColumnIndex()==3){
                    map.put("description",returnStr);
                }else if(c.getColumnIndex()==4){
                    map.put("measurementBasis",returnStr);
                }else if(c.getColumnIndex()==5){
                    map.put("observationMethod",returnStr);
